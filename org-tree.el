...
(defvar org-tree-map
  (let ((map (make-sparse-keymap)))
    ...
    (make-composed-keymap map button-map)))
...
(defun org-tree ()
  ...
  (let* ((buffer (current-buffer))
         (embark-buffer (format "*Embark Live: %s*"
                                (buffer-name buffer)))
         (tree-buffer (format "<tree>%s" (buffer-name buffer)))
         (file-name (file-name-nondirectory buffer-file-name))
         ;;(inhibit-message t)
         min)
    (unless (get-buffer tree-buffer)
      (embark-live)
      (select-window (get-buffer-window embark-buffer))
      (rename-buffer tree-buffer))
